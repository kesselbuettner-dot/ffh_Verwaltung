...
import org.springframework.security.crypto.password.PasswordEncoder; import org.springframework.stereotype.Service; import org.springframework.transaction.annotation.Transactional; import org.springframework.web.server.ResponseStatusException; import org.springframework.http.HttpStatus; import java.math.BigDecimal; import java.util.*; import org.springframework.security.core.Authentication;
@Service public class MemberService {
 private final MemberRepository members; private final AppUserRepository users; private final PasswordEncoder encoder;
 public MemberService(MemberRepository m,AppUserRepository u,PasswordEncoder e){members=m;users=u;encoder=e;}
 @Transactional(readOnly=true) public List<MemberDtos.MemberResponse> all(){return members.findAll().stream().map(this::dto).toList();}
 ...
 private void syncUser(Member m,MemberDtos.MemberRequest r){boolean enabled=Boolean.TRUE.equals(r.loginEnabled());AppUser u=m.getUser();if(!enabled){if(u!=null)u.setEnabled(false);return;}if(u==null){u=new AppUser();u.setMember(m);if(r.password()==null||r.password().isBlank())throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Für einen neuen Login ist ein Passwort erforderlich");}u.setUsername(r.username().trim());u.setRole(r.role());u.setEnabled(m.isActive());if(r.password()!=null&&!r.password().isBlank())u.setPasswordHash(encoder.encode(r.password()));users.save(u);m.setUser(u);}
 private MemberDtos.MemberResponse dto(Member m){AppUser u=m.getUser();return new MemberDtos.MemberResponse(m.getId(),m.getName(),m.getEmail(),m.getPhone(),m.getAddress(),m.isActive(),m.getBalance(),u==null?null:u.getId(),u==null?null:u.getUsername(),u==null?null:u.getRole(),u!=null&&u.isEnabled());}
