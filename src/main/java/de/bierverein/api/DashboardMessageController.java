...
public class DashboardMessageController {
    ...
    public List<WallboardView> wallboard(Authentication auth){
        ...
        return messages.findAllByOrderByPriorityDescEventAtAscCreatedAtDesc().stream()
            .filter(m->("MESSAGE".equals(m.getType())||"APPOINTMENT".equals(m.getType()))&&m.isActive()&&m.isOnWallboard()
              &&(m.getPublishFrom()==null||!m.getPublishFrom().isAfter(now))
              &&(m.getPublishUntil()==null||!m.getPublishUntil().isBefore(now)))
            .map(m->new WallboardView(m.getType(),m.getTitle(),m.getBody(),m.getEventAt(),m.getPriority())).toList();
    }
    public record WallboardView(String type,String title,String body,Instant eventAt,int priority){}
    ...
    private void apply(DashboardMessage m,Request r){if(r==null||r.title()==null||r.title().isBlank())throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Titel ist erforderlich");String type=Optional.ofNullable(r.type()).orElse("MESSAGE").toUpperCase();if(!Set.of("MESSAGE","APPOINTMENT").contains(type))throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Ungültiger Typ");m.setType(type);m.setTitle(trim(r.title(),140));m.setBody(trim(r.body(),2000));m.setEventAt("APPOINTMENT".equals(type)?r.eventAt():null);if("MESSAGE".equals(type)){int days=r.displayDays()==null?7:Math.max(1,Math.min(365,r.displayDays()));Instant start=m.getPublishFrom()==null?Instant.now():m.getPublishFrom();m.setPublishFrom(start);m.setPublishUntil(start.plusSeconds(days*86400L));}else{m.setPublishFrom(null);m.setPublishUntil(null);}m.setPriority(Math.max(0,Math.min(10,r.priority())));m.setActive(r.active());m.setOnWallboard(r.wallboard());}
