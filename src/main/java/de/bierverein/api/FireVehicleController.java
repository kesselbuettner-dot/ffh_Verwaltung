...
public class FireVehicleController {
 ...
 public VehicleDto update(@PathVariable Long id,@RequestBody VehicleRequest r){FireVehicle v=find(id);if(r==null||r.name()==null||r.name().isBlank())throw bad("Fahrzeugname ist erforderlich.");v.setName(trim(r.name(),120));v.setCallSign(trim(r.callSign(),40));v.setNotes(trim(r.notes(),1000));if(r.fireRelevant()!=null)v.setFireRelevant(r.fireRelevant());v.setRequiredLicenseClass(requiredClass(r.requiredLicenseClass()));return dto(vehicles.save(v));}
