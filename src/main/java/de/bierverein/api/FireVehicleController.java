...
public class FireVehicleController {
 ...
 private String requiredClass(String value){
  ...
  if(!LICENSE_CLASSES.contains(normalized))throw bad("Ungültige benötigte Führerscheinklasse");
  return normalized;
 }
 ...
 public VehicleDto create(@RequestBody VehicleRequest r){if(r==null||r.name()==null||r.name().isBlank())throw bad("Fahrzeugname ist erforderlich.");FireVehicle v=new FireVehicle();v.setName(trim(r.name(),120));v.setCallSign(trim(r.callSign(),40));v.setNotes(trim(r.notes(),1000));v.setFireRelevant(!Boolean.FALSE.equals(r.fireRelevant()));v.setRequiredLicenseClass(requiredClass(r.requiredLicenseClass()));return dto(vehicles.save(v));}
 @PutMapping("/{id}") @PreAuthorize("@vehiclePermissionGuard.allowed(authentication,'write')")
 public VehicleDto update(@PathVariable Long id,@RequestBody VehicleRequest r){FireVehicle v=find(id);if(r==null||r.name()==null||r.name().isBlank())throw bad("Fahrzeugname ist erforderlich.");v.setName(trim(r.name(),120));v.setCallSign(trim(r.callSign(),40));v.setNotes(trim(r.notes(),1000));if(r.fireRelevant()!=null)v.setFireRelevant(r.fireRelevant());if(r.requiredLicenseClass()!=null)v.setRequiredLicenseClass(requiredClass(r.requiredLicenseClass()));return dto(vehicles.save(v));}
 ...
 private VehicleDto dto(FireVehicle v){return new VehicleDto(v.getId(),v.getName(),v.getCallSign(),v.getNotes(),v.isFireRelevant(),v.getRequiredLicenseClass(),compartments.findByVehicleIdOrderByGridYAscGridXAsc(v.getId()).stream().map(this::compartmentDto).toList());}
 ...
 public record VehicleRequest(String name,String callSign,String notes,Boolean fireRelevant,String requiredLicenseClass){}
 ...
 public record VehicleDto(Long id,String name,String callSign,String notes,boolean fireRelevant,String requiredLicenseClass,List<CompartmentDto> compartments){}
}
