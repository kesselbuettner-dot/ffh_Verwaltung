...
public class DeviceInspectionPlanningController {
 ...
 @GetMapping("/sessions/{eventId}/{date}") @Transactional public SessionDetail session(@PathVariable Long eventId,@PathVariable LocalDate date){TrainingScheduleEvent e=event(eventId);if(!e.isDeviceInspection()||!occurs(e,date))throw bad("Ungültiger Geräteprüftermin.");Long signedReportId=reports.findByEventIdAndOccurrenceDate(eventId,date).map(DeviceInspectionSessionReport::getId).orElse(null);if(signedReportId==null)ensureTasks(e,date);return new SessionDetail(new SessionDto(e.getId(),date,e.getTitle(),split(e.getDeviceLocations()),split(e.getDeviceCategories()),0,0,signedReportId),tasks.findByEventIdAndOccurrenceDateOrderByDeviceNameAsc(eventId,date).stream().filter(t->eligible(t.getDevice())).map(this::taskDto).toList());}
