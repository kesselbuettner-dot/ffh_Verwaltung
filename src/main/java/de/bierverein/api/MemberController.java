...
@RestController @RequestMapping("/api/members") public class MemberController { private final MemberService service; public MemberController(MemberService s){service=s;}
 ...
 @PostMapping @PreAuthorize("hasAnyRole('ADMIN','VORSTAND')") public MemberDtos.MemberResponse create(@RequestBody MemberDtos.MemberRequest r){return service.create(r);}
 @PutMapping("/{id}") @PreAuthorize("hasAnyRole('ADMIN','VORSTAND')") public MemberDtos.MemberResponse update(@PathVariable Long id,@RequestBody MemberDtos.MemberRequest r){return service.update(id,r);}
