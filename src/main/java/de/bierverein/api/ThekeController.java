...
@RestController @RequestMapping("/api/theke") public class ThekeController {
 ...
 @GetMapping("/drinks") @PreAuthorize("hasAnyRole('ADMIN','THEKE')") public List<DrinkDto> drinks(){return drinks.findAll().stream().filter(Drink::isActive).map(d->new DrinkDto(d.getId(),d.getName(),d.getCategory(),d.getPrice(),d.getEan(),d.getStock(),d.getWarningThreshold())).toList();}
 ...
 public record DrinkDto(Long id,String name,String category,BigDecimal price,String ean,int stock,int warningThreshold){} public record MemberDto(Long id,String name,BigDecimal balance){}
