...
@RestController @RequestMapping("/api/theke") public class ThekeController {
 ...
 @GetMapping("/drinks") @PreAuthorize("hasAnyRole('ADMIN','THEKE','MEMBER')") public List<DrinkDto> drinks(){return articles.findAllByOrderByActiveDescNameAsc().stream().filter(a->a.isActive() && "Getränke".equalsIgnoreCase(a.getCategory())).map(a->new DrinkDto(a.getId(),a.getName(),a.getCategory(),a.getPrice())).toList();}
